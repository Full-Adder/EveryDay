...
root = './Python/pytorchLearn/0507/model/'
...
def get_dataloar(train=True,batch_size = BATCH_SIZE):
...
def test():
    loss_list = []
    acc_list = []
    test_dataload = get_dataloar(False,TEST_BAICH_SIZE)
    for id,(input,label) in enumerate(test_dataload):
        with torch.no_grad():
            output = model(input)
